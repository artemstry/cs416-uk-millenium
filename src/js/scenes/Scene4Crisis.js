...
export class Scene4Crisis {
    constructor(sceneGroup, data, parameters) {
        ...
        // Crisis period data (1900-1950) with null safety
    ...
    addSceneTitle() {
        ...
        this.sceneGroup.append('text')
            ...
            .style('fill', '#1f4e79')
            .text('Crisis & Transformation (1900-1950)');
        ...
        this.sceneGroup.append('text')
            ...
            .style('fill', '#666')
            .text('Wars, Depression, and the Rise of the Modern State');
    }
    ...
    renderTransformationStory(populationData, gdpData) {
        ...
        // Use GDP as primary for Crisis period (shows volatility better)
    ...
    addInteractiveStoryPoints(isPrimaryPopulation) {
        // Crisis & Transformation specific historical events
        const historicalEvents = [
            {
                year: 1914,
                event: 'World War I Begins',
                story: 'Global conflict mobilizes entire economy for total war effort',
                economicEffect: 'Massive government spending, industrial mobilization, but enormous human and economic costs',
                impact: 'devastating',
                y: 80
            },
            {
                ...
                story: 'Stock market crash triggers worst economic downturn in modern history',
                economicEffect: 'Mass unemployment, bank failures, international trade collapse, economic theories challenged',
                impact: 'devastating',
                y: 100
            },
            {
                year: 1936,
                event: 'Keynes General Theory',
                story: 'Revolutionary economic theory advocates government intervention to manage demand',
                economicEffect: 'Provides intellectual foundation for active government economic management',
                impact: 'positive',
                y: 70
            },
            {
                ...
                story: 'Even greater mobilization than WWI, transforming state and economy',
                economicEffect: 'Total war economy, technological advancement, massive state expansion',
                impact: 'social',
                y: 90
            },
            {
                year: 1945,
                event: 'Welfare State Founded',
                story: 'Post-war consensus creates comprehensive social safety net',
                economicEffect: 'Government becomes major economic actor, social spending expands dramatically',
                impact: 'positive',
                y: 110
            }
        ];
        ...
        // Event circles - color-coded by impact
        eventGroup.append('circle')
            .attr('cx', d => this.xScale(d.year))
            .attr('cy', d => d.y)
            .attr('r', 10) // Large for major events
    ...
    addFuturePreview() {
        ...
        const previewText = this.sceneGroup.append('text')
            ...
            .style('cursor', 'pointer')
            .text('Next: Modern Service Economy →')
            .on('click', () => console.log('Navigate to Scene 5'));
    }
    ...
    getPopulationForYear(year) {
        const dataPoint = this.crisisData.data.find(d => d.year === year);
    ...
    estimatePopulation(year) {
        ...
        if (year <= 1920) return 38000;
        if (year <= 1940) return 42000;
        return 46000;
    }
    ...
    getEconomicContext(year) {
        // Crisis & Transformation period context (1900-1950)
        if (year <= 1914) {
            return {
                period: 'Edwardian Peak',
                structure: 'Mature industrial economy at height of imperial power',
                industries: 'Agriculture (15%), Heavy industry (65%), Services and finance (20%)',
                social: 'Social tensions rising, labor movement growing, international competition intensifying'
            };
        } else if (year <= 1920) {
            return {
                period: 'Great War Economy',
                structure: 'Total war mobilization transforming state and economy',
                industries: 'Agriculture (10%), War production (75%), Government services (15%)',
                social: 'Unprecedented state intervention, women entering workforce, social revolution'
            };
        } else if (year <= 1930) {
            return {
                period: 'Post-War Adjustment',
                structure: 'Struggling to return to pre-war normalcy amid global economic instability',
                industries: 'Agriculture (10%), Manufacturing (60%), Services (30%)',
                social: 'General Strike, mass unemployment, return to gold standard difficulties'
            };
        } else if (year <= 1940) {
            return {
                period: 'Great Depression Era',
                structure: 'Economic collapse forcing radical rethinking of government role',
                industries: 'Agriculture (8%), Declining manufacturing (50%), Expanding services (42%)',
                social: 'Mass unemployment, means testing, abandonment of free trade and gold standard'
            };
        } else {
            return {
                period: 'Wartime Transformation',
                structure: 'Second World War creating blueprint for post-war welfare state',
                industries: 'Agriculture (5%), War production (70%), Government services (25%)',
                social: 'Full employment, social solidarity, planning for post-war reconstruction'
            };
        }
    }
    ...
    addEconomicStructure() {
        ...
        const availableYears = this.crisisData.data
        ...
        const xScale = this.xScale || d3.scaleLinear()
            .domain([actualStartYear - 10, Math.max(actualEndYear + 10, 1950)])
            .range([0, this.width]);
        ...
        for (let year = actualStartYear; year <= actualEndYear; year += 5) {
            timePoints.push({
                ...
                government: this.getIndustryPercentage(year, 'government')
            });
        }
        ...
        if ((actualEndYear - actualStartYear) % 5 !== 0) {
            ...
            timePoints.push({
                ...
                government: this.getIndustryPercentage(finalYear, 'government')
            });
        }
        
        // Create stacked area chart using D3's stack generator
        const stack = d3.stack()
            .keys(['agriculture', 'manufacturing', 'government']);
        ...
        const industryNames = ['agriculture', 'manufacturing', 'government'];
        const industryColors = ['#8B4513', '#4682B4', '#9370DB'];  // Brown, Steel Blue, Medium Purple
        const industryFullNames = ['Agriculture', 'Manufacturing', 'Government & Services'];
        ...
        overlay
            .on('mousemove', (event) => {
                ...
                this.showIndustryBreakdownTooltip(event, {
                    ...
                    government: closestData.government
    ...
    getIndustryPercentage(year, industry) {
        // Crisis & Transformation period industry evolution (1900-1950)
        const progress = (year - 1900) / (1950 - 1900); // 0 to 1
        
        switch (industry) {
            case 'agriculture':
                return Math.max(5, 15 - (progress * 10)); // 15% → 5%
            case 'manufacturing':
                return Math.max(50, 65 - (progress * 15)); // 65% → 50%
            case 'government':
                return Math.min(45, 20 + (progress * 25)); // 20% → 45%
            default:
                return 0;
        }
    }
    ...
    addIndustryLegend(startY) {
        const industries = [
            ...
            { name: 'Government & Services', color: '#9370DB' }
        ];
    ...
    showIndustryBreakdownTooltip(event, data, colors, names) {
        ...
        tooltipContent += `<strong>${names[2]}:</strong> ${data.government.toFixed(1)}%</div>`;
        
        // Add total verification
        const total = data.agriculture + data.manufacturing + data.government;
    ...
    renderStagnationStory() {
        this.sceneGroup.append('text')
            ...
            .style('fill', '#666')
            .text('Crisis & Transformation data is being processed...');
    }
    
    exit() {
        console.log('🔄 Scene 4 - Exit called');
